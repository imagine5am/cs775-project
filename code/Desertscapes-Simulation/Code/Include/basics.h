...
public:
	...
	*/
	...
	//QT compatible:
	inline Vector2 Gradient(Quad* qd, char c) const
	{

		Vector2 ret = Vector2(0.0,0.0);

		CellData* curr = qd->fetchData();

		
		std::vector<CellData*> nei = qd->allNeighbors();
		int nn = nei.size();
		...
		//Vector2 dirs[nn];
		//for (int i = 0; i < nn; i++) {
		//	dirs[i] = Normalize(curr.centre - nei[i].centre);
		//	float dist = Magnitude(curr.centre - nei[i].centre);
		//
		//	//finds value at approx at perimeter from centre of subcell by linearly interpolating.
		//	vals[i]= Math::Lerp(nei[i].sediments, curr.sediments, (dist- 1.207106781186548)/dist); //(1+sqrt(2))/2
		//
		//}
		
		

		float perimeter = 2* (curr->sizex + curr->sizey);
		//std::cerr << "chkpt2.3 --- Grad 1.1" << std::endl;
		//float temp=0;

		//if (i==0.0 || i== float(nx) - (curr->sizex)/2)  //no cell to the left  (xor right)
		//	temp += perimeter / 4;
		//if (j == 0.0 || j == float(ny) - (curr->sizey) / 2)  //no cell on the top (xor bottom)
		//	temp += perimeter / 4;

		//perimeter -= temp;

		Vector2 cTR = curr->topRight;
		Vector2 cBL = curr->botLeft;
		...
		for (int i = 0; i < nn; i++) {
			...
			if (nei[i]->topRight.x == cBL.x || nei[i]->botLeft.x == cTR.x) {
				...
				contact = fmin(nei[i]->topRight.x, cTR.x) - fmax(nei[i]->botLeft.x, cBL.x);
				if (nei[i]->topRight.y == cBL.y) { //nei on the bottom edge
					ret.y += (contact / perimeter) * (curr->vals[c] - nei[i]->vals[c]);
				}
				else {
					ret.y -= (contact / perimeter) * (curr->vals[c] - nei[i]->vals[c]);
				}
		...
		ret.x /= float(curr->sizex);
		ret.y /= float(curr->sizey);
